...
fun NavGraph(
    ...
    LinearBackground {
        NavHost(navController = navController, startDestination = sDestination) {
            ...
            animatedComposable(route = Screens.ProfileScreen.route) {
                ProfileScreen(onLogoutClick = {
                    sharedViewModel.onEvent(SharedUiEvent.Logout)
                    navController.navigateAndClearStack(Screens.AuthNavigator.route)
            ...
            animatedComposable(route = Screens.LogScreen.route) {
            ...
            animatedComposable(route = Screens.WorkTimeScreen.route) {
            ...
            animatedComposable(route = Screens.SalaryScreen.route) {
            ...
            animatedComposable(route = Screens.PayCheckDetail.route) {
                val paycheck =
            ...
            animatedComposable(route = Screens.ShiftScreen.route) {
            ...
            animatedComposable(route = Screens.MyProfileScreen.route) {
                Log.d("viewmodel_home", "composable my profile")
                MyProfileScreen(state = sharedViewModel.state, onBack = {
                    navController.popBackStack()
                })
            }
            animatedComposable(route = Screens.SettingScreen.route) {
                SettingScreen(onBack = {
                    navController.popBackStack()
                })
            }
            animatedComposable(route = Screens.TermConditionScreen.route) {
                TermConditionScreen(onBack = {
                    navController.popBackStack()
                })
            }
            animatedComposable(route = Screens.PrivacyScreen.route) {
